...
[![GitHub release](https://img.shields.io/github/v/release/hix83/BYDMate-DM?style=flat-square)](https://github.com/hix83/BYDMate-DM/releases)
...
[О форке](#о-форке-bydmate-dm) | [Возможности](#возможности) | [Скриншоты](#скриншоты) | [Автоматизация](#автоматизация) | [Вывод на приборку](#вывод-на-приборку) | [HUD](#hud) | [Разделение экрана](#разделение-экрана) | [Голосовой агент](#голосовой-ai-агент) | [AI Инсайты](#ai-инсайты) | [ABRP](#abrp--live-телеметрия) | [Установка](#установка) | [Сборка](#сборка-из-исходников) | [Релизы](https://github.com/hix83/BYDMate-DM/releases) | [Поддержать](SUPPORT.md)
...
Этот репозиторий — рабочий форк [основного BYDMate](https://github.com/AndyShaman/BYDMate), адаптированный под BYD Song L DM-i / DiLink 5.0 и реальные проверки на установленной голове. Текущая линия `3.14.3-dm` основана на upstream `v3.14.3`.

Форк сохраняет поверх upstream только особенности нашей машины:

- гибридный профиль Song L DM-i, расход топлива и общую стоимость поездки по электричеству и бензину;
- fallback вывода навигатора через скрытый display `2`, `ClusterAnchorActivity` и helper-daemon;
- парковочные IP-камеры и управление шлагбаумами;
- адаптивное масштабирование интерфейса в компактных окнах DiLink.

Общие возможности и исправления регулярно подтягиваются из основного репозитория, а DM-зависимые изменения выпускаются отдельно в [BYDMate-DM Releases](https://github.com/hix83/BYDMate-DM/releases).
...
| **Bat** | Здоровье батареи и «Техника» | Карточка: SoH (на Leopard 3), температура, 12V, разброс ячеек, изоляция. Тап открывает экран «Техника» с живыми параметрами батареи, моторов, климата и шин |
...
Тап по карточке открывает экран **«Техника»**: напряжение и ток тяговой батареи, доступная мощность батареи, лимиты BMS на заряд и разряд, разброс ячеек, температуры моторов и инверторов, климат, давление и температура шин, история SoH и пробега. У каждого параметра есть значок «?» с подсказкой простым языком. Если машина не отдаёт параметр, строка показывает «-», пустые карточки скрываются.
